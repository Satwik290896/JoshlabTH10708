...
public class TrialController : MonoBehaviour {
	...
	void Start(){
		...
		Forward.GetComponent<Image>().enabled = false;
		Forward.GetComponent<Button>().enabled = false;
		Forward.GetComponent<LongClickStraight>().enabled = false;
		Left.GetComponent<Image>().enabled = false;
		Left.GetComponent<Button>().enabled = false;
		Left.GetComponent<LongClickButton>().enabled = false;
		Right.GetComponent<Image>().enabled = false;
		Right.GetComponent<Button>().enabled = false;
		Right.GetComponent<LongClickRR>().enabled = false;
	}
	...
	IEnumerator RunTrial(Trial trial){
		...
		Forward.GetComponent<Image>().enabled = true;
		Forward.GetComponent<Button>().enabled = true;
		Forward.GetComponent<LongClickStraight>().enabled = true;
		Left.GetComponent<Image>().enabled = true;
		Left.GetComponent<Button>().enabled = true;
		Left.GetComponent<LongClickButton>().enabled = true;
		Right.GetComponent<Image>().enabled = true;
		Right.GetComponent<Button>().enabled = true;
		Right.GetComponent<LongClickRR>().enabled = true;
		currentTrial = trial;
		...
		for (int i = 0; i < exp.objectController.CurrentTrialSpecialObjects.Count; i++) {
			...
			if (ExperimentSettings_CoinTask.myLanguage == ExperimentSettings_CoinTask.LanguageSetting.Spanish) {
				...
				if (splitName [0] == "el") {
					...
					selectObjectText = selectObjectText + "l" + " " + displayNameNoEl + " (Drag the Ring)."; //add the 'l' to "de" to make "del"
				}
				else {
					selectObjectText = selectObjectText + " " + specialItemDisplayName + " (Drag the Ring).";
				}
			} 
			else { //english
				selectObjectText = selectObjectText + " " + specialItemDisplayName + " (Drag the Ring).";
			...
			UnityEngine.Debug.Log("SelectionSelectionSelection: sfewjwi: Length: " + chosenPositions.Count);
			/*for (i = 0; i < chosenPositions.Count; i++) {
				UnityEngine.Debug.Log("SelectionSelectionSelection: sfewjwi: " + chosenPositions[i]);
			}*/
	...
	IEnumerator ShowFeedback(List<int> specialObjectOrder, List<Vector3> chosenPositions, List<Config_CoinTask.MemoryState> rememberResponses){
		UnityEngine.Debug.Log("SelectionSelectionSelection: sfewjwi: Count: " + chosenPositions.Count);
